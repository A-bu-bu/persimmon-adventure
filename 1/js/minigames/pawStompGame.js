// Mini-Game 1: 蟡萱?喃葦憭批?瘙?(Lucky Paw Stomp)
...
export class PawStompGame {
  ...
  draw() {
    ...
    ctx.textAlign = 'left';
    ctx.fillText('? 蟡萱?喃葦憭批?瘙?(Lucky Paw Stomp)', 30, 40);

    ctx.fillStyle = '#fff';
    ctx.font = 'bold 16px "Microsoft JhengHei", sans-serif';
    ctx.fillText(`?梧? ??: ${Math.ceil(this.timeLeft)}s`, 30, 70);
    ctx.fillText(`潃?敺?: ${this.score}`, 200, 70);
    if (this.combo > 1) {
      ctx.fillStyle = '#ffeb3b';
      ctx.fillText(`? ???: x${this.combo}!`, 380, 70);
    }
    ...
    for (let s of this.spots) {
      ...
      ctx.textAlign = 'center';
      ctx.fillText(`[? ${s.key}]`, s.x, s.y + s.radius + 20);
      ...
      if (s.state) {
        ...
        if (s.state === 'golden_paw') {
          ...
          ctx.fillStyle = '#ffeb3b';
          ctx.fillText('憟賡??箸靘?', s.x, s.y - 34);
        } else if (s.state === 'monster_foot') {
    ...
    if (this.isOver) {
      ...
      ctx.textAlign = 'center';
      ctx.fillText('?? 頦抵銝急??啁?蝞?', w / 2, h / 2 - 50);

      ctx.fillStyle = '#fff';
      ctx.font = 'bold 20px "Microsoft JhengHei", sans-serif';
      ctx.fillText(`蝮賢??? ${this.score} ?? |  ?擃??: x${this.maxCombo}`, w / 2, h / 2);
